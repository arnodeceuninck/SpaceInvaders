...
spaceinvaders::model::RocketModel::RocketModel(const spaceinvaders::Coordinate &speedDirection,
        ...
        : MovingEntity(speedDirection, position), damage(damage) {
}
...
void spaceinvaders::model::RocketModel::update(double elapsedSeconds) {
    ...
    std::shared_ptr<spaceinvaders::event::Event> event =
            std::make_shared<spaceinvaders::event::RocketPositionUpdated>(
...
double spaceinvaders::model::RocketModel::getDamage() const { return damage; }

void spaceinvaders::model::RocketModel::setDamage(double damage) { RocketModel::damage = damage; }
